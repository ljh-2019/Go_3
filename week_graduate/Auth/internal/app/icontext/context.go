...
// NewAccountKey 创建账号标识的上下文
func NewAccountKey(ctx context.Context, accountKey string) context.Context {
	return context.WithValue(ctx, accountKeyCtx{}, accountKey)
}

// FromAccountKey 从上下文中获取账号标识
func FromAccountKey(ctx context.Context) (string, bool) {
	v := ctx.Value(accountKeyCtx{})
	if v != nil {
		if s, ok := v.(string); ok {
			return s, s != ""
		}
